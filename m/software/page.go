...
func NewPages() *Pages {
  return &Pages{
                 a: make(map[string]Page),
               }
}

func (s *Pages) FileExtension() string {
  return ".page"
}

func (s *Pages) Count() int64 {
  return int64(len(s.a))
}

func (s *Pages) Append(info *Page) {
  info.CODE = strings.ToLower(info.CODE)
  s.a[info.CODE] = *info
}

func (s *Pages) GetByCODE(code string) (*Page) {
  item, ok := s.a[code]
  if ok {
    return &item
  }
...
func (s *Pages) LoadFromFiles(scanPath string) int {
  count := 0
...
func (s *Pages) FileParse(filename string, jsonFile []byte) int {
  var err error
  var oTmp Page
